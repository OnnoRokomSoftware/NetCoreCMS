...
namespace NetCoreCMS.Modules.Analytics
{
    public class AnalyticsModule : IModule
    {
        ...
        {
            services.AddTransient<AnalyticsRepository>();
            services.AddTransient<AnalyticsLogRepository>();
            
            services.AddTransient<AnalyticsService>();
            services.AddTransient<AnalyticsLogService>();
        }
        ...
        {
            var createQuery = @"
                CREATE TABLE IF NOT EXISTS `Ncc_Analytics` ( 
                ...
                CREATE TABLE IF NOT EXISTS `Ncc_Analytics_log` ( 
                    `Id` BIGINT NOT NULL AUTO_INCREMENT , 
                    `VersionNumber` INT NOT NULL , 
                    `Metadata` varchar(250) COLLATE utf8_unicode_ci NULL,
                    `Name` VARCHAR(250) NOT NULL , 
                    ...
                    `VisitUrl` VARCHAR(2000) NULL , 
                    `ReferrerUrl` VARCHAR(2000) NULL , 
                    `Browser` VARCHAR(250) NULL , 
                    `BrowserVersion` VARCHAR(250) NULL , 
                    `Platform` VARCHAR(250) NULL , 
                    `IsMobile` bit(1) NOT NULL , 
                    `MobileDeviceModel` VARCHAR(250) NULL , 
                    `IpAddress` VARCHAR(250) NULL , 
                    `BrowserAgent` longtext NULL , 

                    `AnalyticsModelId` BIGINT NOT NULL , 
                PRIMARY KEY (`Id`)) ENGINE = MyISAM;
            ";
        ...
        {
            var deleteQuery = @"
                DROP TABLE IF EXISTS `Ncc_Analytics_log`;
                DROP TABLE IF EXISTS `Ncc_Analytics`;
            ;";
